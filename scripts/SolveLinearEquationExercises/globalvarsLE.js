...
function LEincrementColor(somenumber){
//parameter = index being used to keep track of colors used. 

//var x = somenumber;
//x = (x + 1)%COLOR_Array.length;

var x = Math.ceil(Math.random()*(COLOR_Array.length - 1));

if (x != somenumber){ 
	return x; 
}

return LEincrementColor(somenumber);
}
...
var BRACKETED_CONSTANT_pattern = '\\(' + '[+\\-]?'+ POS_INT + '\\)' ;
