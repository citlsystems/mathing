...
var numberOfLines = stevesString.match(/&=/g).length ;  console.log("Number of lines: ",numberOfLines);
...
for (var i=0; i<strangeChars.length; i++){
    if (stevesString.indexOf(strangeChars[i])== -1){
        var separator = strangeChars[i] ; console.log(separator);
        stevesString = stevesString.replace(/\\\\\[1mm\]/g,strangeChars[i]);  console.log(stevesString);
        break;
        }
...
for (var i=0;i<numberOfLines; i++){
        console.log(linearEquationREGEX.lastIndex);
...
a = eqn.split("=")[side].replace(/\(\([^()]+\)[wxyz]\)/g,"0");
a = a.replace(/\(|\)/g,"");
a = a.split("+") ;
...
// FIX:  NOT WORKING WITH DEGENERATE CASES!!!!
 
console.log('Var Terms LEFT : ', LEvarTermsLP(eqn,'left'), ' ---SUM: ', LEsumFractionArray(LEvarTermsLP(eqn,'left')));  
console.log('Var Terms RIGHT: ', LEvarTermsLP(eqn,'right'), ' ---SUM: ', LEsumFractionArray(LEvarTermsLP(eqn,'right')));
console.log('var LEFT - var RIGHT:  ', LEsubtractFracs( LEsumFractionArray(LEvarTermsLP(eqn,'left')),LEsumFractionArray(LEvarTermsLP(eqn,'right'))));
...
console.log("constcoeff: ----- ", constcoeff);
 
if (parseInt(xcoeff)==0){
    if (parseInt(constcoeff)==0){
...
var eqnblah = LEredress(someLinearEquation); console.log(eqnblah);
...
if (/\(\([^-()]+\)[xwyz]?\)/.test(eqnblah))
    {this.numposterms = eqnblah.match(/\(\([^-()]+\)[xwyz]?\)/g).length ;}
    else {this.numposterms = 0;}
if (/\(\(-[^()]+\)[xwyz]?\)/.test(eqnblah))
    {this.numnegterms = eqnblah.match(/\(\(-[^()]+\)[xwyz]?\)/g).length ;}
...
else {signa = 9999; console.log("error signa:   ",signa);}
...
else {signb = 9999;console.log("error signb:   ",signb);}
...
else {sign1 = 9999;console.log("error sign1:   ",sign1);}
...
encouragement[2] = "So far, so good.";
encouragement[3] = "How did you get so good at this?";
...
/ current line (eqn1) are detectable. NOTE: eqn1 and eqn2 are objects defined by
 equationLP(), and NOT strings  */
function LEsetErrorFlags(eqn1, eqn2){
 
if (eqn1.solution!="no solution" && eqn2.solution!="no solution" && eqn1.solution!="all real numbers" && eqn1.solution!="all real numbers"){
    if (!LEequivalent(eqn1.solution,eqn2.solution)){
        errorflag[0]= true;}
    }
else if (eqn1.solution == "no solution" && eqn2.solution!= "no solution"){
        errorflag[0] = true;}
else if (eqn1.solution == "all real numbers" && eqn2.solution != "all real numbers"){
        errorflag[0] = true;}
...
//Generates the appropriate string to be displayed in the FEEDBACK BAR. 
...
var builderStringPAR = builderPARAMETER.replace(/([^&])=/g, '$1&='); //puts the & in the equations?
 
if (builderStringPAR.match(/&=/g).length < 2){  //this case should be impossible since the listener can't be activated until there are at least two equations to compare (ie, the length has to be >= 2)	
    message = "Give it a try and I'll let you know how you\'re doing!" ;
    }
 
else {
var arrayOfLines539 = LEstoreLinesLE(builderStringPAR) ;  console.log('ARRAY OF '+arrayOfLines539.length+' STORED LINES: ', arrayOfLines539);
var i = arrayOfLines539.length;
previouseqnPAR = arrayOfLines539[i-2];
currenteqnPAR = arrayOfLines539[i-1] ;
 
var previouseqnLP = new LEequationLP(previouseqnPAR);
var currenteqnLP = new LEequationLP(currenteqnPAR);
...
if (allgoodflag) {
	...
	if (LEisLinearEquationSolution(currenteqnPAR)){
