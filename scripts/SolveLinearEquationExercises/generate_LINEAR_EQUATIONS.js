...
STEVE -> Main function to be called is "randNiceIntLinTexEq()"
*/
...
//	equation = LErandNiceRatLinTexEq('x', 2, 2, coeffset);
    equation = LErandNiceIntLinTexEq2('x', 2, 2, coeffset);   //put back rational version when fraction stuff done
...
//	equation = LErandNiceRatLinTexEq(use_var, numtermschoices[Math.floor(numtermschoices.length*Math.random())], numtermschoices[Math.floor(numtermschoices.length*Math.random())], coeffset);
	equation = LErandNiceIntLinTexEq2(use_var, numtermschoices[Math.floor(numtermschoices.length*Math.random())], numtermschoices[Math.floor(numtermschoices.length*Math.random())], coeffset);
...
//STEVE
function LErandomNiceIntegerTerm(var_parameter, set_of_coeffs_to_choose_from_parameter){
var x, a,b,c,d,e;

if (LEcoinflip()){a="("; e=")";b = LErandomSign();} else {a=""; b = ""; e="";}
c = set_of_coeffs_to_choose_from_parameter[Math.floor(set_of_coeffs_to_choose_from_parameter.length*Math.random())];
if (LEcoinflip()){d="";} else {d=var_parameter;}

x = a+b+c+d+e;
return x;
}

function LErandomNiceIntegerConstantTerm(var_parameter, set_of_coeffs_to_choose_from_parameter){
var x, a,b,c,d,e;
...
function LErandomNiceIntegerVariableTerm(var_parameter, set_of_coeffs_to_choose_from_parameter){
var x, a,b,c,d,e;
...
function LErandomNiceRationalTerm(varParameter, set_of_coeffs_parameter){
...
var array_of_numbers = set_of_coeffs_parameter; //just helps to make the coefficients look nicer to deal with
...
function LErandomNiceRationalConstantTerm(varParameter, set_of_coeffs_parameter){
//generates a linear term in the variable varParameter, with rational coefficients
	var x,a,b,numerator, denominator,d,e;
...
function LErandomNiceRationalVariableTerm(varParameter, set_of_coeffs_parameter){
//generates a linear term in the variable varParameter, with rational coefficients
	var x,a,b,numerator, denominator,d,e;
...
function LErandNiceIntLinTexEq2(var_parameter, terms_parameterL, terms_parameterR, coeffset_parameter){
//if only 2 terms per side, will be of the form ax+b = cx+data

var numTermsL = terms_parameterL; //prompt('Enter number of terms to appear on each side:', 3);
var numTermsR = terms_parameterR;

var x=LErandomNiceIntegerVariableTerm(var_parameter, coeffset_parameter); //left side of the equation
var y=LErandomNiceIntegerVariableTerm(var_parameter, coeffset_parameter); //right side of the equation
var z = "";

x = x + LEplusORminus() +LErandomNiceIntegerConstantTerm(var_parameter, coeffset_parameter);
y = y + LEplusORminus() +LErandomNiceIntegerConstantTerm(var_parameter, coeffset_parameter);

for (var i=2; i < numTermsL; i++){
x = x + LEplusORminus() +LErandomNiceIntegerTerm(var_parameter, coeffset_parameter);
}
for (var i=2; i < numTermsR; i++){
y = y + LEplusORminus() +LErandomNiceIntegerTerm(var_parameter, coeffset_parameter);
}

/*-- MAKE SURE NOT NO SOLUTION:
...
//
function LErandNiceRatLinTexEq(varParameter, number_terms_left_PAR, number_terms_right_PAR, coeff_set_parameter){

//var maxInt = max_int_PAR; //prompt('Enter max integer value:', 20);
var numTermsL = number_terms_left_PAR ; //prompt('Enter number of terms to appear on each side:', 3);
var numTermsR = number_terms_right_PAR ;

var x=LErandomNiceRationalVariableTerm(varParameter, coeff_set_parameter); //left side of the equation
var y=LErandomNiceRationalVariableTerm(varParameter, coeff_set_parameter); //right side of the equation
var z = "";

x = x + LEplusORminus() +LErandomNiceRationalConstantTerm(varParameter, coeff_set_parameter);
y = y + LEplusORminus() +LErandomNiceRationalConstantTerm(varParameter, coeff_set_parameter);

for (var i=2; i < numTermsL; i++){
x = x + LEplusORminus() +LErandomNiceRationalTerm(varParameter, coeff_set_parameter);
}
for (var i=2; i < numTermsR; i++){
y = y + LEplusORminus() +LErandomNiceRationalTerm(varParameter, coeff_set_parameter);
}
